...
@MODELS.register_module(force=True)  # 将当前类注册为模型模块
...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        ...
        # 【修复】动态截断 attention mask 匹配输入长度，避免 77x77 mask 与短序列不匹配
        for block in self.transformer.resblocks:
            if hasattr(block, "attn_mask") and block.attn_mask is not None:
                if block.attn_mask.shape[0] != seq_len:
                    block.attn_mask = block.attn_mask[:seq_len, :seq_len].clone()
